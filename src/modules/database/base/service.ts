...
> {
    ...
    async detail(id: string, callback?: QueryHook<E>): Promise<E> {
        ...
        if (!item) throw new NotFoundException(`${this.repository.qbName} not exists!`);
        return item;
    }
     ...
     * 批量删除数据
     * @param ids 数据ID
     * @param trash 是否放入回收站, 如果为true则为软删除
     * */
        ...
        if (this.repository instanceof BaseTreeRepository) {
            ...
            if (this.repository.childrenResolve === TreeChildrenResolve.UP) {
                for (const item of items) {
                    if (isNil(item.children) || item.children.length < 1) continue;
    ...
    create(data: any, ...others: any[]): Promise<E> {
        throw new ForbiddenException(`Can not create ${this.repository.qbName}!`);
    }
    ...
    update(data: any, ...others: any[]): Promise<E> {
        throw new ForbiddenException(`Can not update ${this.repository.qbName}!`);
    }
}
